...
if command -v psql >/dev/null 2>&1; then
  ...
  HAVE_PSQL=0
  echo "!! WARNING: 'psql' command not found; running in offline mode (commands will be printed only)."
fi

psqlx(){
  if [[ $HAVE_PSQL -eq 0 ]]; then
    echo "SKIP> psql $* (psql not available)"
    return 1
  fi
  psql "$PGURL" -v ON_ERROR_STOP=1 -X -q "$@"
}
...
FROM need;
EOS
    if DDLs=$(psqlx -t -A -c "$IDX_SQL"); then
      if [[ -n "$DDLs" ]]; then
      ...
      echo "   (index introspection failed; see warning above)"
    fi
  else
    echo "   (index introspection skipped: psql not available)"
  fi
fi
...
if [[ -n "$MV_NAME" && -n "$MV_SQL" ]]; then
  ...
  run psqlx -c "REFRESH MATERIALIZED VIEW CONCURRENTLY $SCHEMA.$MV_NAME;" || psql_show -c "REFRESH MATERIALIZED VIEW $SCHEMA.$MV_NAME;"
fi

# 3) VERTICAL SCALING — print right-size hints (no config writes by default)
echo "-> Vertical scaling hints:"
psql_show -c "SELECT name, setting FROM pg_settings WHERE name IN ('shared_buffers','work_mem','maintenance_work_mem','effective_cache_size');"
...
if [[ -n "$TARGET_TABLE" ]]; then
  run psqlx -c "VACUUM (ANALYZE) $SCHEMA.$TARGET_TABLE;"
fi
psql_show -c "SELECT query, calls, total_time, mean_time FROM pg_stat_statements ORDER BY total_time DESC LIMIT 10;"
if [[ -n "$TEST_SQL" ]]; then
